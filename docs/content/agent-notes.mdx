...
Codex writes short notes for each changed file by default. Diff Presenter sends
the selected diff to the Codex CLI under its current login.

Use a plain diff when you do not want notes:

```sh
npx diff-presenter --no-agent
```
...
Other targets use separate files in Diff Presenter’s ignored
