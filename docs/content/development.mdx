...
```sh
corepack pnpm run package:verify
```

This local command checks the built tarball without publishing it. The
production audit has no findings. `pnpm audit` reports three high and one low
finding below the Blume docs tool. We accept this limited risk while docs builds
use only committed docs and do not handle user content.
