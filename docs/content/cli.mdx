...
| `--jobs COUNT` | Set agent passes to run at once, from `1` to `8`. OpenCode always runs one pass at a time. |
...
then OpenCode. It stops with an error if none are installed.
